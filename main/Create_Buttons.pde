...
void Create_Buttons() {
  ...
  Position = cp5.addSlider("positon").setPosition(x/2 - x/4, y/22 - y)
    ...
    public void controlEvent(CallbackEvent theEvent) {
      if (theEvent.getAction() == ControlP5.ACTION_ENTER) {
        MouseRepositioner.mouseMove(mouseX, 480);
        if (theEvent.getAction() == ControlP5.ACTION_ENTER) {
          println("ding");
          Position.setValue((mouseX - (x/2 - x/4))/(x/2) * 7011);
        }
      }
    }
  }
  ...
  test = cp5.addButton("test").setPosition(x - x/8, -y/8)
    ...
    new CallbackListener() {
    public void controlEvent(CallbackEvent theEvent) {
      if (theEvent.getController().isMouseOver() == true && theEvent.getController().isActive() == true && theEvent.getController().isBroadcast() == true) {
        MouseRepositioner.mouseMove(800, 480);
        theEvent.getController().setMousePressed(true);
        theEvent.getController().setMousePressed(false);
      }
  ...
  Winder_Speed_Knob = cp5.addSlider("Winder_Speed_Knob").setPosition(x/2 - x/5, y/2 + y/4 - y)
    ...
    public void controlEvent(CallbackEvent theEvent) {
      if (theEvent.getAction() == ControlP5.ACTION_ENTER) {
        MouseRepositioner.mouseMove(mouseX, 480);
        if (theEvent.getAction() == ControlP5.ACTION_ENTER) {
          Winder_Speed_Knob.setValue((mouseX - (x/2 - x/5))/(x/3 + x/15) * (20000/67));
        }
      }
    }
  }
  ...
  Carriage_Speed_Knob = cp5.addSlider("Carriage_Speed_Knob").setPosition(x/2 - x/5, y/2 - y/4 - y)
    ...
    public void controlEvent(CallbackEvent theEvent) {
      if (theEvent.getAction() == ControlP5.ACTION_ENTER) {
        MouseRepositioner.mouseMove(mouseX, 480);
        if (theEvent.getAction() == ControlP5.ACTION_ENTER) {
          Carriage_Speed_Knob.setValue((mouseX - (x/2 - x/5))/(x/3 + x/15) * (20000/67));
        }
      }
    }
  }
